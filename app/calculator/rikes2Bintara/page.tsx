...
const Rikes2Bintara = () => {
  ...
  const handleCalculate = () => {
    if (!scores.academic || !scores.psychology) {
      setError('Semua bidang harus diisi!');
      return;
    }

    const academicScore = (parseFloat(scores.academic) || 0) * 0.6;
    const psychologyScore = (parseFloat(scores.psychology) || 0) * 0.4;

    const total = academicScore + psychologyScore;
  ...
  return (
    ...
    <div className="flex flex-col bg-white rounded-xl shadow-lg mt-6 w-full sm:w-4/5 md:w-3/5 xl:w-2/5">
        <div className='flex flex-col p-6 space-y-1'>
          <h1 className="font-semibold tracking-tight text-xl">Kalkulator Rikes 2 - BINTARA PTU</h1>
        </div>
        ...
        {showResult && (
          <div>
            ...
            <div className='flex flex-col gap-4 p-6'>
              ...
              <div>
                <p>Nilai Akademik (60%) : {result.academicScore.toFixed(1)}</p>
                <p>Nilai Psikologi (40%) : {result.psychologyScore.toFixed(1)}</p>
              </div>
            </div>
